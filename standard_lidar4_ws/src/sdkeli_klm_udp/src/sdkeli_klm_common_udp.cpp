...
CSDKeliKlmCommonUdp::CSDKeliKlmCommonUdp(const std::string &hostname, const std::string &port, int &timelimit, CParserBase *parser, ros::NodeHandle &nh, ros::NodeHandle &pnh) :
    CSDKeliKlmCommon(parser, nh, pnh),
    mHostName(hostname),
    mPort(port),
...
CSDKeliKlmCommonUdp::~CSDKeliKlmCommonUdp()
{
    StopScanner();
    CloseDevice();
}

int CSDKeliKlmCommonUdp::InitDevice()
{
    int opt = 1;
...
int CSDKeliKlmCommonUdp::CloseDevice()
{
    if(mSocket != -1)
    {
        close(mSocket);
...
int CSDKeliKlmCommonUdp::SendUdpData2Device(char *buf, int length)
{
    int n = -1;
    if(mSocket > 0)
    {
...
int CSDKeliKlmCommonUdp::SendDeviceReq(const char *req, std::vector<unsigned char> *resp)
{
    if(mSocket == -1)
    {
        ROS_ERROR("SendDeviceReq: Socket NOT open");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "SDKELI_KLM - SendDeviceReq: socket NOT open!");
    ...
    if(SendUdpData2Device((char *)req, SIZE_OF_CMD) != SIZE_OF_CMD)
    {
        ...
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "SDKELI_KLM - SendDeviceReq: Write command failed!");
...
int CSDKeliKlmCommonUdp::GetDataGram(unsigned char *receiveBuffer, int bufferSize, int *length)
{
    int len;
    struct timeval tv;
    socklen_t addrlen;
    ...
    if(mSocket == -1)
    {
        ...
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "SDKELI_KLM - GetDataGram: socket NOT open!");
    ...
    if(1)//(!stream_stopped_)
    {
        ...
        {
            std::string errorStr =  "SDKELI_KLM - GetDataGram: No full response for read after " + mTimeLimit;
            errorStr = errorStr + "S";
