...
class SiminicsLaserProtocol : public LaserProtocolInterface {
 public:
    struct SiminicsScanPackage {
        ...
        uint16_t angle_start; // 雷达的角度坐标系和我们采用的不一致, 雷达角度坐标系下，减去180度
        uint16_t angle_end;
    ...
    SiminicsLaserProtocol():last_sensor_send_time(-1) {}
    ...
    virtual bool cpToScan(std::shared_ptr<ScanMsg>& scan) {
        ...
        auto angle_range = (simin_scan.angle_end - simin_scan.angle_start) * ANGLE_UNIT;
#ifdef ROS_NODE
        scan->header.stamp = ros::Time::now();
        scan->header.frame_id = "laser";
#else
        scan->time_ = sync_time - angle_range / 2 / 360.0 * scan->scan_time * 1e6; // unit us, calculate middle time. received end scan time.
#endif
        scan->angle_increment = simin_scan.angle_resolution * ANGLE_UNIT *M_PI / 180.0; // 0.08, 0.16, 0.32
        scan->angle_min = (simin_scan.angle_start * ANGLE_UNIT - 180) * M_PI / 180.0;
        scan->angle_max = (simin_scan.angle_end * ANGLE_UNIT - 180) * M_PI / 180.0;
        scan->time_increment = 1.0 / (SCAN_FREQ * simin_scan.point_num * 360 / angle_range);
        scan->range_min = 0.01;
        scan->range_max = 120;
    ...
    virtual bool checkScanValid() {
        if (sensor_send_time != last_sensor_send_time) {
            last_sensor_send_time = sensor_send_time;
            return true;
        }else{
            LOG(INFO) << "get same time!" << last_sensor_send_time << "," << sensor_send_time;
        }
        return false;
    }
 ...
 private:
    bool parseConfig(boost::circular_buffer<char>& ring_buffer){
        ...
        if (ring_buffer.size() >= simin_scan.point_num * 4 + 4 + PACKAGE_CONFIG_SIZE) {
            ring_buffer.erase_begin(PACKAGE_CONFIG_SIZE);
            return true;
        }
        return false;
    }
    ...
    bool parseScanData(boost::circular_buffer<char>& ring_buffer){
        ...
        readBufferFront(ring_buffer, package_data_bytes.data(), simin_scan.point_num * 4 + 4);
        ring_buffer.erase_begin(simin_scan.point_num * 4 + 4);
